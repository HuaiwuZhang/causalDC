...
class deepsynergy(BaseModel):
    def __init__(self, gene_dim,
                 **args
                 ):
        super(deepsynergy, self).__init__(**args)
    ...
    def forward(self, drugA, drugB, cline_gene, cline_mutation, cline_dependency, cline_copynumber):
        ...
        cell1 = cell1.reshape(cell1.size(0), -1)
        if self.args.get_cell_embedding:
            cell_embedding = cell1
            return cell_embedding

        combined = torch.cat([drugA, drugB, cell1], dim=1)
